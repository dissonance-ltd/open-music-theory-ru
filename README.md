...
Доступны черновики введения и первых двадцати глав Fundamentals: введение в западную
...
и хроматическая гамма; пение с листа и музыкальный диктант; интервалы; трезвучия.
В книге 21 черновик и 104 локальные иллюстрации (103 из Nebraska и одна из LibreTexts). Внешние видео, партитуры и рабочие листы пока
...
оглавление Nebraska и 22 английских снимка: двадцать одна переводимая страница и благодарности.
Пакет 06 добавляет главы 15–17 на основе восстановленного пакета 05.
Пакет 05 уже восстановлен в `main` через [PR #10](https://github.com/dissonance-ltd/open-music-theory-ru/pull/10). [PR #11](https://github.com/dissonance-ltd/open-music-theory-ru/pull/11) был объединён в прежнюю ветку восстановления; эта ветка переносит его проверенные главы 15–17 и расширение MuseScore непосредственно в `main`. Подробности: [upstream/README.md](upstream/README.md).

Переводы подготовлены с помощью ИИ и требуют независимой терминологической и
содержательной рецензии. В `upstream/translations.json` у всех восемнадцати страниц статус
...
Для глав 15–17 выполнены отдельные проверки ИИ и проверены исправления;
результаты зафиксированы в записях глав. Технические результаты — там же и в PR.
...
- Провести независимое рецензирование восемнадцати черновиков и глоссария.
- Проверить работу внешних упражнений и видео, затем локализовать рабочие листы.
- Следующий объём начинается с главы 18; исполнители пока не назначены, очередь — в [трекере](docs/chapter-tracker.md).
...
## Интерактивные нотные примеры

Все 60 исходных партитур MuseScore в переведённых главах 6–17 встроены в страницы. Под каждым плеером сохранена прямая ссылка. Плеерам нужен интернет; содержимое остаётся на языке оригинала. Пользователь подтвердил работу пилота; поэлементная проверка всех партитур остаётся отдельной задачей. [Инвентарь и правила для следующих глав](docs/musescore-embeds.md).

Пакет 07: [септаккорды](src/fundamentals/18-seventh-chords.md), исходные страницы и изображения из Nebraska Fall 2023; инвентарь и независимая ИИ-сверка в [записи главы](docs/reviews/seventh-chords.md). Внешние партитуры и задания остаются на английском.

Пакет 08: [обращения и цифровой бас](src/fundamentals/19-inversion-figured-bass.md) и [римские цифры и SATB](src/fundamentals/20-roman-numerals.md). Отдельные ИИ-сверки и неполные элементы источника — в [записях](docs/reviews/inversion-figured-bass.md) [глав](docs/reviews/roman-numerals.md).
